...
void SalaryCount::showCompanyDialog(QAction* actionEmited)
{
    ...
    QMessageBox::information(NULL,QString::fromWCharArray(L"Ёто первый релиз"),QString::fromWCharArray(L"‘ункции переключени€ предпри€тий недоступны.\n¬ насто€щее врем€ ¬ы можете работать с одним фиксированным предпри€тием."));
}
