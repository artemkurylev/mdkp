...
bool Employee::update() const
{
    if(DbManager::manager().checkConnection())
    {
        ...
        query->prepare("UPDATE `employee` SET `fio` = :fio , `phone_number` = :phone_number, `inn` = :inn, `hire_directive_id` = :hire_directive_id, `dutychart_id` = :dutychart_id, `next_dutyсhart_id` = :next_dutyсhart_id, `next_dutychart_since` = :next_dutychart_since WHERE `id` = id");
...
bool Employee::createDbTable()
{
    ...
    {
        QSqlQuery* query = DbManager::manager().makeQuery();
        if(query->exec("CREATE TABLE IF NOT EXISTS `employee` (`id` INT(11) NOT NULL AUTO_INCREMENT, `fio` CHAR(30) NOT NULL, `phone_number` CHAR(20),`inn` DECIMAL(10),`hire_directive_id` INT(11),`dutychart_id` INT(11),`next_dutychart_id` INT(11),`next_dutychart_since` DATE, PRIMARY KEY(`id`))"))
		{
			success = true;
		}
        else
