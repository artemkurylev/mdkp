...
void salarycountEmployees::clearFields()
{
	...
	catch(log_errors::exception_states e)
	{
		...
		{
			switchMode(app_states::USUAL);//откат к состо€нию просмотра
		}
	}
}
...
bool salarycountEmployees::fillDutyChartComboBox(PayForm pf)
{
	try
	{
		if(this->currentPayForm!=pf || !ui->eDutyChart->count())
		{
			...
			if(!countDutyChart) throw this->journal->notFound("Ќе удалось получить графики работы, возможно они не созданы");

			QMap<int,QString> &charts = DutyChart::getAllByPayForm(pf);

			if(!charts.count()) throw this->journal->invalidData("√рафики не созданы.\nѕеред добавлением нового сотрудника необходимо создать графики работы");
...
{
	this->currentState = state;//запомним состо€ние приложени€
	bool isAddState = state==app_states::ADD;

	//приведем состо€ние к булеву типу дл€ посылки главному окну
	...
	emit changeState(triggerState);//посылка состо€ни€ в родительское окно
}
...
void salarycountEmployees::saveNewEntries(Employee* obj)
{
	try
	{
		...
		if(this->currentState != app_states::ADD) 
			throw this->journal->compareError("«начени€ состо€ни€ дл€ сохранени€ не совпадает с состо€нием приложени€");
...
void salarycountEmployees::saveEditableEntries(Employee* obj)
{
	try
	{
		...
		if(this->currentState != app_states::EDIT) 
			throw this->journal->compareError("«начени€ состо€ни€ дл€ сохранени€ не совпадает с состо€нием приложени€");
		if(!obj) throw this->journal->nullPtr("ќбъект дл€ сохранени€ не существует");
		if(!obj->validate()) throw this->journal->validateError("¬алидаци€ записи не успешна");
		if(!obj->update()) throw this->journal->updateError("«апись не дошла в бд");
